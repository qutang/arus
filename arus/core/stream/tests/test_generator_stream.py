...
def test_GeneratorSlidingWindowStream():
    ...
    stream = GeneratorSlidingWindowStream(
        data_source=config, window_size=window_size, start_time=None, start_time_col=0, stop_time_col=0, name='sensor-generator-stream')
    stream.start()
    chunk_sizes = []
    chunk_means = []
    n = 5
    for data, _, _, _, _, name in stream.get_iterator():
        ...
        if n == 0:
            break
    assert np.all(np.array(chunk_sizes[1:-1]) == window_size * config['kwargs']['sr'])
    ...
    stream = GeneratorSlidingWindowStream(
        data_source=config, window_size=window_size, start_time=None, start_time_col=1, stop_time_col=2, name='annotation-generator-stream')
    stream.start()
    chunk_durations = []
    n = 5
    for data, _, _, _, _, name in stream.get_iterator():
        ...
        if n == 0:
            break
    np.testing.assert_array_equal(np.array(chunk_durations) <= window_size, True)